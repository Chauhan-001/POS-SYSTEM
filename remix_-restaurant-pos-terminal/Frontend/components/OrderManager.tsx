...
export default function OrderManager({
  ...
  const renderTableCard = (table: TableInfo) => {
    ...
    const statusStyle = TABLE_STATUS_COLORS[table.status] || TABLE_STATUS_COLORS['Available'];
    const bill = getRunningBill(table.id);
    const sectionColor = table.section ? (sectionColors[table.section] || 'bg-gray-100 text-gray-700 border-gray-200') : '';

    return (
      <div
        key={table.id}
        className={`relative rounded-2xl border-2 transition-all duration-200 group
          ${statusStyle.bg} 
          ${table.status === 'Available' 
            ? 'border-gray-200 hover:border-emerald-400 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer' 
      ...
      >
        ...
        {table.priority === 'urgent' && (
          <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full z-10 animate-pulse">
            URGENT
          </span>
        )}

        {/* Billing icon always visible on occupied tables */}
        {table.status !== 'Available' && order && (
          <>
            <button
              onClick={(e) => { e.stopPropagation(); onOpenReceiptPreview(order); }}
              className="absolute top-2 right-10 z-20 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:shadow-lg hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer border border-blue-200 hover:border-blue-400"
            ...
            <button
              ...
              className="absolute top-2 right-2 z-20 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:shadow-lg hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer border border-orange-200 hover:border-orange-400"
              title="Open billing"
            >
              <Receipt className="w-4 h-4 text-orange-500 hover:text-orange-600" />
            </button>
          </>
        )}
          ...
          <div className="p-3.5 pb-2">
            <div className="flex justify-between items-start mb-1">
              <div className="flex items-center gap-2">
                <span className="text-lg font-black text-gray-900">T{table.number}</span>
                {table.status !== 'Occupied' && (
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${statusStyle.text} bg-white/80 border ${statusStyle.dot.replace('bg-', 'border-')}/30`}>
                    {table.status}
                  </span>
                )}
              </div>
            ...
            {table.section && (
              <span className={`inline-block px-2 py-0.5 rounded text-[8px] font-bold mt-1 ${sectionColor}`}>
                {table.section}
              </span>
            )}
          </div>
          ...
          <div className="px-3.5 pb-2 space-y-1.5">
              ...
              <>
                {table.orderSince && (
                  <div className="flex items-center gap-1.5 text-[10px] text-gray-500">
                    ...
                    <span className="font-mono">{getElapsedTime(table.orderSince)}</span>
                  </div>
                )}
                {bill > 0 && (
                  <div className="flex items-center gap-1.5 text-[11px] font-black text-gray-800 mt-1 pt-1.5 border-t border-gray-200/60">
                    <span>{settings.currencySymbol || '₹'}{bill.toFixed(2)}</span>
                  </div>
                )}

                {order && order.status === 'Ready' && (
                  <div className="flex items-center gap-1 mt-1 bg-green-100 text-green-800 text-[8px] font-bold px-2 py-0.5 rounded-full w-fit">
                    <CheckCircle className="w-2.5 h-2.5" />
                    FOOD READY
                  </div>
                )}
                {order && order.status === 'Preparing' && (
                  <div className="flex items-center gap-1 mt-1 bg-amber-100 text-amber-800 text-[8px] font-bold px-2 py-0.5 rounded-full w-fit">
            ...
            {table.status === 'Available' && (
              <div className="flex items-center justify-center py-3">
                <span className="text-[11px] font-semibold text-gray-400">Tap to seat guest</span>
              </div>
            )}

            {table.reservationName && (
              <div className="flex items-center gap-1.5 text-[10px] text-blue-700 mt-1 bg-blue-50 rounded-lg px-2 py-1">
                <span className="font-semibold">📋 {table.reservationName}</span>
              </div>
            )}
          </div>
        </div>
  ...
  const renderTakeawayCard = (order: TakeawayOrder) => {
    ...
    // Highlight card red when elapsed time exceeds 30 minutes
    const createdTime = new Date(order.createdAt).getTime();
    const elapsedMin = (Date.now() - createdTime) / 60000;
    ...
    return (
      ...
      >
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="text-[10px] text-gray-500 font-mono">{getElapsedTime(order.createdAt)}</span>
        </div>
        ...
        <div className="flex justify-between items-center pt-2 border-t border-gray-100">
          <span className="font-bold text-sm">
            {settings.currencySymbol || '₹'}{order.amount.toFixed(2)}
          </span>
  ...
  // Render Online Order Card
  const renderOnlineOrderCard = (order: Order) => {
    ...
    return (
      ...
      >
          ...
          {order.customerName && (
            <div className="flex items-center gap-1.5 text-[11px] text-gray-700">
              ...
              <span className="font-semibold">{order.customerName}</span>
                            ...
                            <Clock className="w-3 h-3 text-gray-400" />
                            <span className="font-mono">{getElapsedTime(order.createdAt)}</span>
                          </div>
          {order.deliveryEta && (
        ...
        <div className="flex justify-between items-center pt-2 border-t border-gray-100">
          <span className="font-bold text-sm">
            {settings.currencySymbol || '₹'}{order.grandTotal.toFixed(2)}
          </span>
  ...
  return (
    <div className="h-full flex flex-col bg-[#f8f6f3] overflow-hidden">
      {/* Header section */}
          ...
          <div>
            ...
            {takeawayOrders.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {takeawayOrders.map(renderTakeawayCard)}
              </div>
            )}
          </div>
        )}
            ...
            {(['Swiggy', 'Zomato', 'Uber Eats', 'Website', 'Phone Orders'] as const).map(platform => {
              ...
              return (
                <div key={platform}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {platformOrders.map(renderOnlineOrderCard)}
                  </div>
                </div>
              );
            })}
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(order => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-sm">{settings.currencySymbol || '₹'}{order.grandTotal.toFixed(2)}</span>
