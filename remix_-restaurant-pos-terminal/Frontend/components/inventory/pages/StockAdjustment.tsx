...
export default function StockAdjustment() {
  ...
  const { addStock, removeStock } = useInventory();
  ...
  const actionItems = INVENTORY_ITEMS.filter(i => i.status === 'low' || i.status === 'critical');

  const handleSubmit = () => {
    const item = INVENTORY_ITEMS.find(i => i.id === formItem);
    if (!item || !formQty || parseFloat(formQty) <= 0) {
      notify('Select an item and enter quantity', 'warning');
      return;
    }
    const qtyNum = parseFloat(formQty);
    if (formType === 'add') addStock(item.name, qtyNum);
    else removeStock(item.name, qtyNum);
    setAdjustments(prev => [{
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1">
                ...
                >
                  ...
                  {INVENTORY_ITEMS.map(i => <option key={i.id} value={i.id}>{i.name} ({i.currentStock} {i.unit})</option>)}
                </select>
              </div>
