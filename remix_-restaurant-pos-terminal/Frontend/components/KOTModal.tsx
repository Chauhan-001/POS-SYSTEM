...
export default function KOTModal({
  isOpen, onClose, order, kotRecords, onPrintKOT, onReprintKOT, settings, currentEmployee
}: KOTModalProps) {
  const receiptRef = useRef<HTMLDivElement>(null);

  if (!isOpen || !order) return null;

  // Get items that haven't been printed in a KOT yet
  const printedItemIds = new Set<string>();
  kotRecords.forEach(kot => {
    kot.items.forEach(item => printedItemIds.add(item.id));
  });

  const pendingItems = order.items.filter(item => !printedItemIds.has(item.id));
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col border border-gray-200 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* KOT Preview Section */}
          <div ref={receiptRef} className="bg-gray-50 border border-gray-200 rounded-xl p-5 text-center">
            <h3 className="text-[10px] font-bold uppercase text-gray-500 mb-3">KOT Preview</h3>
            <div className="bg-white border border-dashed border-gray-300 max-w-xs mx-auto p-4 font-mono text-[9px] text-left">
              <div className="text-center border-b border-dashed border-gray-300 pb-2 mb-2">
                <p className="font-bold text-[11px]">{settings.restaurantName || 'Restaurant'}</p>
                <p className="text-gray-500 mt-1">KITCHEN ORDER TICKET</p>
              </div>
              <div className="space-y-0.5 mb-2">
                <p>Order #: {order.orderNumber}</p>
                <p>KOT #: {nextKotNumber} {kotRecords.length > 0 ? '(Additional)' : '(Original)'}</p>
                <p>Date: {new Date().toLocaleDateString()} {new Date().toLocaleTimeString()}</p>
                {order.tableNumber && <p>Table: T{order.tableNumber}</p>}
                {order.waiterName && <p>Waiter: {order.waiterName}</p>}
              ...
              <div className="border-t border-dashed border-gray-300 pt-2">
                ...
                {(pendingItems.length > 0 ? pendingItems : order.items).map(item => (
                  <div key={item.id} className="flex justify-between py-0.5 text-[9px]">
                    <span>
                      {item.product.name}
                      {item.selectedVariant && <span className="text-gray-500"> ({item.selectedVariant.name})</span>}
                    </span>
                    <span>x{item.quantity}</span>
                  </div>
                ))}
              </div>
              {(pendingItems.filter(i => i.notes).length > 0) && (
                <div className="mt-2 text-[8px] text-amber-600 border-t border-dashed border-gray-300 pt-1">
                  {pendingItems.filter(i => i.notes).map(i => (
                    <p key={i.id}>📝 {i.product.name}: {i.notes}</p>
                  ))}
                </div>
              )}
              <div className="border-t border-dashed border-gray-300 mt-2 pt-2 text-center text-[8px] text-gray-500">
                <p>{settings.kotFooterNote || 'Thank you!'}</p>
                {kotRecords.length > 0 && (
                  <p className="text-amber-600 font-bold mt-1">*** ADDITIONAL KOT ***</p>
                )}
              </div>
            </div>
          </div>
