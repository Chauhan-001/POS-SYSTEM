 ...
 *
 * costIntelligenceCore — LAYER: CORE / ML-READY
 *
 * Deterministic, ML-ready restaurant cost metrics (Phases R/S/T/U/D).
 * Everything here is computed from real tenant data server-side — never
 * LLM-invented. Produces the structured metrics bundle a future ML service
 * can consume directly:
 *   productProfitability · ingredientCostChanges · wastage · variance ·
 *   offerEconomics · contribution ranks · cost-change alerts
 *
 * No model inference belongs in this module. FUTURE AI INTEGRATION POINT:
 * AI may narrate the bundle produced here (see costIntelligenceService), but
 * the numbers themselves are authoritative and computed only in this file.
 */
...
export class CostIntelligenceCoreService {
  /**
  ...
  /**
   * Deterministic insights over the metrics bundle — the guard rails AND the
   * fallback for any AI narrative layer. Every sentence references only
   * numbers computed in this file (never fabricated).
   */
  deterministicInsights(m: Awaited<ReturnType<CostIntelligenceCoreService['metrics']>>): string[] {
    ...
    for (const o of (m.offerEconomics || []).filter((o: any) => o.warningCount > 0).slice(0, 2)) {
...
export const costIntelligenceCore = new CostIntelligenceCoreService();
