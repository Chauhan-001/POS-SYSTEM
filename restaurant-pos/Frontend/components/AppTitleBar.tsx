...
export default function AppTitleBar({
  ...
  return (
    <div className="bg-[var(--color-sidebar-bg)] text-white px-4 py-2 flex justify-between items-center text-xs select-none border-b border-[var(--color-sidebar-border)] shrink-0">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer ${
                isBranchMenuOpen
                  ? 'bg-[var(--color-sidebar-border)] ring-1 ring-[#2563eb]/60'
                  : 'bg-[var(--color-sidebar-border)] hover:bg-[var(--color-sidebar-border)]'
              }`}
            >
            ...
            {isBranchMenuOpen && (
              <div className="absolute top-full left-0 mt-1.5 w-52 bg-[#252732] border border-[var(--color-sidebar-border)] rounded-xl shadow-xl z-50 overflow-hidden">
                <div className="p-1.5">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[10px] font-medium transition-colors cursor-pointer ${
                        branch.id === currentBranchId
                          ...
                          : 'text-gray-300 hover:bg-[var(--color-sidebar-border)]'
                      }`}
                    >
                      {branch.isHeadBranch ? (
      ...
      <div className="flex items-center gap-2.5 text-gray-300">
        ...
        <span className="font-mono text-gray-300 bg-[var(--color-sidebar-border)] px-2 py-0.5 rounded-md">{currentTime.toLocaleTimeString()}</span>
        ...
        {isOnline && pendingSyncCount > 0 && (
          <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30 text-[9px] font-bold tracking-wider">
