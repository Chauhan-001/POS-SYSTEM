      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            className="w-full pl-9 pr-9 py-2.5 bg-white border border-[#e1e2ed] rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all"
        ...
        <div className="flex gap-1.5 flex-wrap">
          <button onClick={() => setCategoryFilter('All')}
            className={`px-3.5 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${categoryFilter === 'All' ? 'bg-[#004ac6] text-white shadow-sm' : 'bg-white border border-[#e1e2ed] text-gray-600 hover:border-[#004ac6]/30'}`}
          >All</button>
          {categories.map(c => (
            <button key={c} onClick={() => setCategoryFilter(c)}
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${categoryFilter === c ? 'bg-[#004ac6] text-white shadow-sm' : 'bg-white border border-[#e1e2ed] text-gray-600 hover:border-[#004ac6]/30'}`}
            >{c}</button>
          ))}
          <button onClick={() => { setShowNewCatInput(true); setNewCatName(''); }}
            className="px-3 py-2 rounded-xl text-xs font-semibold text-[#004ac6] border border-dashed border-[#004ac6]/30 hover:bg-blue-50 transition-all cursor-pointer"
          >+ New</button>
        </div>
      </div>
      ...
      {showNewCatInput && (
        <div className="flex items-center gap-2 mb-6">
          <input type="text" value={newCatName} onChange={e => setNewCatName(e.target.value)}
            className="flex-1 max-w-xs px-4 py-2.5 rounded-xl border border-[#004ac6] text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20"
          ...
          <button onClick={() => addCategory(newCatName)}
            className="px-4 py-2.5 bg-[#004ac6] text-white rounded-xl text-xs font-bold hover:bg-[#003ea8] transition-all cursor-pointer">Add</button>
          ...
          >
            {filtered.map((item, i) => (
              <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: i * 0.03 }}
                className="bg-white rounded-2xl border border-[#e1e2ed] overflow-hidden hover:shadow-lg hover:border-[#004ac6]/20 transition-all group"
              >
                {/* Color stripe */}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-[#e1e2ed]">
                    <button onClick={() => openEdit(item)}
                      className="flex-1 py-2 border border-[#e1e2ed] rounded-xl text-[10px] font-semibold text-gray-500 hover:border-[#004ac6]/30 hover:text-[#004ac6] hover:bg-blue-50 transition-all cursor-pointer flex items-center justify-center gap-1"
                    >
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={e => updateField('name', e.target.value)}
                className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all ${formErrors.name ? 'border-red-300 bg-red-50' : 'border-[#c3c6d7] bg-white'}`}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {showNewCatInput ? (
                  <div className="flex gap-2">
                    <input type="text" value={newCatName} onChange={e => setNewCatName(e.target.value)}
                      className="flex-1 px-4 py-3 rounded-xl border border-[#004ac6] text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20"
                      placeholder="New category name..." autoFocus
                      onKeyDown={e => { if (e.key === 'Enter') addCategory(newCatName); if (e.key === 'Escape') { setShowNewCatInput(false); setNewCatName(''); }}} />
                    <button onClick={() => addCategory(newCatName)}
                      className="px-3 py-3 bg-[#004ac6] text-white rounded-xl text-xs font-bold hover:bg-[#003ea8] transition-all cursor-pointer">Add</button>
                  ...
                  <div className="flex gap-2">
                    <select value={form.category} onChange={e => {
                      ...
                      className="flex-1 px-4 py-3 rounded-xl border border-[#c3c6d7] text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] bg-white">
                      {categories.map(c => <option key={c} value={c}>{c}</option>)}
                      <option value="__new__" className="text-[#004ac6] font-semibold">+ Add new…</option>
                    </select>
                  </div>
                )}
              </div>
              <div>
                ...
                <select value={form.unit} onChange={e => updateField('unit', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-[#c3c6d7] text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] bg-white">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" min="0" value={form.currentStock} onChange={e => updateField('currentStock', e.target.value)}
                  className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all ${formErrors.currentStock ? 'border-red-300 bg-red-50' : 'border-[#c3c6d7] bg-white'}`} />
              ...
              <div>
                ...
                <input type="number" min="0" value={form.minStock} onChange={e => updateField('minStock', e.target.value)}
                  className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all ${formErrors.minStock ? 'border-red-300 bg-red-50' : 'border-[#c3c6d7] bg-white'}`} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" min="0" step="0.01" value={form.averageCost} onChange={e => updateField('averageCost', e.target.value)}
                  className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all ${formErrors.averageCost ? 'border-red-300 bg-red-50' : 'border-[#c3c6d7] bg-white'}`} />
              ...
              <div>
                ...
                <input type="text" value={form.supplier} onChange={e => updateField('supplier', e.target.value)}
                  className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[#004ac6]/20 focus:border-[#004ac6] transition-all ${formErrors.supplier ? 'border-red-300 bg-red-50' : 'border-[#c3c6d7] bg-white'}`}
