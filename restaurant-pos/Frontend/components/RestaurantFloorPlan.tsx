          ...
          <div className="bg-[var(--color-bg-white)] rounded-2xl shadow-2xl w-[95vw] max-w-6xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex border-b border-[var(--color-border-default)] shrink-0">
              ...
              <button onClick={() => { if (areas.length === 0) { showToast?.('Create at least one area first', 'warning'); return; } setEditTab('tables'); }}
                className={`flex items-center gap-2 px-5 py-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer ${editTab === 'tables' ? 'border-[var(--brand-color)] text-[var(--brand-color)]' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
              ...
              <div className="flex-1 p-4 bg-[var(--color-bg-page)] flex flex-col min-w-0">
                <div className="flex items-center justify-between mb-2.5">
                  <div className="flex items-center gap-2 text-[10px] text-gray-500">
                    {editTab === 'areas' ? (
                      <><Layers className="w-3 h-3 text-[var(--brand-color)]" /><span>Drag zones · resize via handles · drag entrance · double-click canvas to add</span></>
                    ) : (
                      <><Move className="w-3 h-3 text-[var(--brand-color)]" /><span>Drag to move · 8 handles to resize · double-click empty space to quick-add</span></>
                    )}
                  </div>
                </div>
                  ...
                  >
                    ...
                    {canvasSections.map(s => {
                      ...
                      <div key={s.id}
                        ...
                        className={`absolute rounded-xl border-2 ${s.border} ${s.bg} ${editTab === 'areas' ? 'cursor-grab group ring-2 ring-[var(--brand-color)]/10 hover:ring-[var(--brand-color)]/40 touch-none' : 'pointer-events-none'} ${isAreaDrag ? 'cursor-grabbing z-50 shadow-xl ring-[var(--brand-color)]' : ''} ${isResize ? 'z-50 ring-[var(--brand-color)]' : ''}`}
                        style={{ left: `${s.x}%`, top: `${s.y}%`, width: `${Math.max(8, s.width)}%`, height: `${Math.max(10, s.height)}%` }}
                      >
                        {editTab === 'areas' && (
                          <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`relative flex flex-col items-center justify-center bg-[var(--color-bg-white)] shadow-md transition-all
                            ...
                            ${isDrag ? 'shadow-2xl scale-110 ring-[var(--brand-color)]' : ''}
                            ${isResizing ? 'ring-[var(--brand-color)] shadow-xl' : ''}
                            ...
                            {isEditing ? (
                              <div className="flex items-center gap-1.5">
                                <input type="text" value={editAreaLabel} onChange={e => setEditAreaLabel(e.target.value)}
                                  ...
                                  className="flex-1 px-2 py-1 rounded border border-[var(--brand-color)] text-[10px] font-semibold focus:outline-none" autoFocus />
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-0.5">
                                  <button onClick={() => { setEditingAreaId(area.id); setEditAreaLabel(area.label); }}
                                    className="w-6 h-6 rounded-md flex items-center justify-center text-gray-400 hover:text-[var(--brand-color)] hover:bg-blue-50 transition-all cursor-pointer"
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex gap-1">
                                <button onClick={() => setAddForm({ ...addForm, shape: 'round' })}
                                  className={`flex-1 py-1.5 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center gap-1 ${addForm.shape === 'round' ? 'bg-[var(--brand-color)] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                ><Circle className="w-2.5 h-2.5" />R</button>
                                <button onClick={() => setAddForm({ ...addForm, shape: 'rect' })}
                                  className={`flex-1 py-1.5 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex items-center justify-center gap-1 ${addForm.shape === 'rect' ? 'bg-[var(--brand-color)] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                ...
                                >
                                  <div className="flex items-center gap-2 min-w-0">
                                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center font-bold text-[10px] shrink-0
                                      ${isSelected ? 'bg-[var(--brand-color)] text-white' : table.status === 'Available' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-700'}`}>
                                      {isEditingNum ? (
                                        <input type="number" value={editingTableNumVal}
                                          onChange={e => setEditingTableNumVal(e.target.value)}
                                          onKeyDown={e => { if (e.key === 'Enter') updateTableNumber(table.id); if (e.key === 'Escape') setEditingTableNum(null); }}
