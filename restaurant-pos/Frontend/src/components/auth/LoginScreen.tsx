...
import { normalizeRole, type Employee } from '../../types';
import { getSavedAccounts, removeAccount, type SavedAccount } from '../../savedAccounts';
...
export default function LoginScreen({ onLoginSuccess, settings, employees = [], loginMethod = 'password', isFirstRun = false, onRegister }: LoginScreenProps) {
  ...
  const checkConsentForUsername = (u: string) => {
    if (u.trim() && isConsentedFor(u.trim().toLowerCase())) {
      ...
      // A manually-ticked box must survive typing — the common order is
      // "tick the consent, then type the username", and resetting the tick
      // on every keystroke would lock the user out of signing in. Only
      // withdraw the tick when switching AWAY from a previously consented
      // account (its auto-tick was account-specific).
      setAcceptedTerms((prev) => (prev && !consentPersisted ? prev : false));
    }
  };
